...
log=${TOPDIR}/sig-validation.log
echo "Starting test at $(date +%H%M%S)" | tee -a $log
...
do
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            echo "Artifact validation failed on iteration $count" | tee -a $log
            savefile=${DST}-$(date +%H%M%S)
            echo "Saving as $savefile" | tee -a $log
            mv ${DST} ${savefile}
        fi
    else
        echo "Artifact creation failed on iteration $count" | tee -a $log
        echo "Failure time $(date +%H%M%S)" | tee -a $log
    fi
done
